...
function applyAutoAppendicitisFields(text, type) {
  ...
  const metricMappings = [
    ...
    ["creatinine", ["CREA", "CR"], ["肌酐"]], ["albumin", ["ALB"], ["白蛋白"]],
    ["totalBilirubin", ["TBIL"], ["总胆红素"]], ["glucose", ["GLU"], ["葡萄糖", "血糖"]],
  ];
...
const APPENDICITIS_FIELD_GROUPS = [
  {
    ...
    alwaysOpen: true,
    fields: [
      { key: "studyId", label: "研究编号", type: "text", placeholder: "如 APP-0001" },
      { key: "admissionDate", label: "入院日期", type: "date" },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "rlqTenderness", label: "右下腹压痛", type: "select", options: APPENDICITIS_VALUE_OPTIONS },
      { key: "reboundGuarding", label: "反跳痛或肌紧张", type: "select", options: APPENDICITIS_VALUE_OPTIONS },
  ...
  },
  {
    ...
    fields: [
      { key: "admissionTime", label: "入院时间", type: "datetime-local" },
  ...
  },
  {
    ...
    types: ["出院小结"],
    fields: [
      { key: "dischargeDate", label: "出院日期", type: "date" },
...
async function recognizeWithPaddle() {
  ...
  try {
    ...
    const preliminaryText = buildOcrTextFromBlocks(blocks);
    const preliminaryPerson = detectPerson(preliminaryText, blocks);
    const preliminaryMetrics = labFieldRules[els.recordType.value] ? extractAllLabRows(preliminaryText, blocks) : [];
    const labThreshold = els.recordType.value === "血常规" ? 12 : 4;
    const needsFocused = (!preliminaryPerson.name && !preliminaryPerson.personId)
      || (Boolean(labFieldRules[els.recordType.value]) && preliminaryMetrics.length < labThreshold);
    if (needsFocused) {
      ...
      if (focusedVariants.length) {
        els.processingDetail.textContent = `快速识别结果不足，补跑 ${focusedVariants.length} 个重点区域`;
...
function getResearchPatientId(record) {
  const studyId = String(record?.appendicitisData?.studyId || "").trim();
...
function buildResearchWorkbookRows() {
  ...
  orderedRecords.forEach((record, index) => {
    const patientKey = personKey(record);
    ...
    if (String(record?.appendicitisData?.studyId || "").trim()) patient.patientId = patientId;
